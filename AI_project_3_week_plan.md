...
- [x] Create the GitHub repo today, even empty. Commit from day one — the visible history is
  part of the point given your profile's been quiet.
- [x] Spin up Postgres locally (Docker is easiest) and load a sample dataset.
  - Done with Docker Compose, PostgreSQL, and the Chinook dataset.
- [x] Get an API key, send one hello-world request to the LLM from a Python script.
  - Done through the OpenAI-powered SQL generator.
- [x] Write your **test set**: 8–10 real questions you want the app to answer, from easy
...
- [x] **Days 1–2 — The core loop (no UI).** One script: take a hardcoded question, send the
...
- [x] **Day 4 — Wrap it in FastAPI.** One endpoint: `POST /ask` taking `{question}` and
...
- [x] **Day 1 — Evals.** Write a small script that runs all 8–10 test questions and reports
